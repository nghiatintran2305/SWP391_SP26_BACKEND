...
import com.example.swp391.integrations.service.IntegrationSyncService;
...
public class IntegrationConfigServiceImpl implements IIntegrationConfigService {
    ...
    public SyncJobResponse sync(String groupId, SyncRequest request) {
        ...
        SyncJob job = SyncJob.builder()
                ...
                .createdBy(new SecurityUtil(accountRepository).getCurrentAccount())
                .build();

        SyncJob saved = syncJobRepository.save(job);
        IntegrationSyncService.SyncSummary summary = runSync(request, config);

        saved.setStatus(SyncJobStatus.SUCCESS);
        saved.setFinishedAt(LocalDateTime.now());
        saved.setMessage("Sync completed");
        syncJobRepository.save(saved);

        config.setLastSyncAt(saved.getFinishedAt());
        configRepository.save(config);

        SyncJobResponse response = mapToResponse(saved);
        response.setJiraIssueCount(summary.jiraIssueCount());
        response.setJiraSprintCount(summary.jiraSprintCount());
        response.setGithubCommitCount(summary.githubCommitCount());
        return response;
    }

    private IntegrationSyncService.SyncSummary runSync(SyncRequest request, IntegrationConfig config) {
        return switch (request.getType()) {
            case JIRA -> integrationSyncService.syncJira(config);
            case GITHUB -> integrationSyncService.syncGithub(config);
            case FULL -> {
                IntegrationSyncService.SyncSummary jiraSummary = integrationSyncService.syncJira(config);
                IntegrationSyncService.SyncSummary githubSummary = integrationSyncService.syncGithub(config);
                yield new IntegrationSyncService.SyncSummary(
                        jiraSummary.jiraIssueCount(),
                        jiraSummary.jiraSprintCount(),
                        githubSummary.githubCommitCount()
                );
            }
        };
    }
